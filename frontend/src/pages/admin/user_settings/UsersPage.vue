 ...
 */
const getAllRecords = computed((): PaginatedResultsInterface => userStore.getAllRecords);

/**
 * Computed function that retrieves and processes filters from the userStore.
...
/**
 * Computed function that retrieves a single user record from the userStore.
 * @returns {UserInterface} An object representing a single user record.
 */
const getSingleRecord = computed((): UserInterface => userStore.getSingleRecord);
...
async function clearFilter(filterKey: string): Promise<void> {
  ...
  if (savedSearch && savedSearch !== null) {
    ...
    removeFilterFromLocalStorage.value(userStore.$id, existingSearchQuery, filterKey)
