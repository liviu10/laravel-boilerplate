...
import { useContactStore } from 'src/stores/admin/communication/contact';
...
const getAllRecords = computed(() => contactStore.getAllRecords);
...
const getSingleRecord = computed(() => contactStore.getSingleRecord);
...
function saveRecord() {
  ...
  contactStore.createRecord().then(() => {
    loadData.value = false
  })
}
...
function editRecord() {
  ...
  if (recordId) {
    loadData.value = true
    displayActionDialog.value = false
    contactStore.updateRecord(recordId).then(() => {
      loadData.value = false
    })
  } else {
    const notificationTitle = t('admin.generic.notification_warning_title');
...
function deleteRecord() {
  ...
  if (recordId) {
    loadData.value = true
    displayActionDialog.value = false
    contactStore.deleteRecord(recordId).then(() => {
      loadData.value = false
    })
  } else {
    const notificationTitle = t('admin.generic.notification_warning_title');
...
onMounted(async () => {
  loadData.value = true
  await contactStore.getRecords().then(() => {
    loadData.value = false
  })
})
</script>
