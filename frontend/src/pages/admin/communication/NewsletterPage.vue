...
const getSingleRecord = computed(() => newsletterStore.getSingleRecord);
...
async function actionMethodDialog(action: DialogType, recordId?: number) {
  ...
  if (action === 'create') {
    ...
    if (isInvalidRecordId) {
      ...
      newsletterStore.findRecord(recordId).then(() => {
        loadData.value = false
        actionName.value = action
...
function handleActionMethod(action: DialogType) {
  ...
  loadData.value = true
  if (action === 'create') {
    newsletterStore.createRecord().then(() => {
      loadData.value = false
    })
  } else {
    const recordId = getSingleRecord.value && Object.keys(getSingleRecord.value).length > 0 ? getSingleRecord.value.id : null;
    if (recordId) {
      if (action === 'edit') {
        newsletterStore.updateRecord(recordId).then(() => {
          loadData.value = false
        })
      } else if (action === 'delete') {
        newsletterStore.deleteRecord(recordId).then(() => {
          loadData.value = false
        })
      }
    } else {
...
onMounted(async () => {
  loadData.value = true
  await newsletterStore.getRecords().then(() => {
    loadData.value = false
  })
})
</script>
