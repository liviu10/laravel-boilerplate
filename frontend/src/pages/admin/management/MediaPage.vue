...
import TableColumns from 'src/columns/mediaColumns';
...
const getAllRecords = computed(() => mediaStore.getAllRecords);
...
const getSingleRecord = computed(() => mediaStore.getSingleRecord);
...
async function actionMethodDialog(action: DialogType, recordId?: number) {
  ...
  if (action === 'create') {
    ...
    if (isInvalidRecordId) {
      ...
      mediaStore.findRecord(recordId).then(() => {
        loadData.value = false
        actionName.value = action
...
onMounted(async () => {
  loadData.value = true
  await mediaStore.getRecords().then(() => {
    loadData.value = false
  })
})
</script>
