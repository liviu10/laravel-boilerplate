...
import TableColumns from 'src/columns/commentsColumns';
...
import { useCommentStore } from 'src/stores/admin/management/comments';
...
const getAllRecords = computed(() => commentStore.getAllRecords);
...
const getSingleRecord = computed(() => commentStore.getSingleRecord);
...
async function actionMethodDialog(action: ActionMethodDialogType, recordId?: number) {
  ...
  if (action === 'create') {
    ...
    if (isInvalidRecordId) {
      ...
      commentStore.findRecord(recordId).then(() => {
        loadData.value = false
        actionName.value = action
...
function saveRecord() {
  ...
  commentStore.createRecord().then(() => {
    loadData.value = false
  })
}
...
function editRecord() {
  ...
  if (recordId) {
    loadData.value = true
    displayActionDialog.value = false
    commentStore.updateRecord(recordId).then(() => {
      loadData.value = false
    })
  } else {
    const notificationTitle = t('admin.generic.notification_warning_title');
...
function deleteRecord() {
  ...
  if (recordId) {
    loadData.value = true
    displayActionDialog.value = false
    commentStore.deleteRecord(recordId).then(() => {
      loadData.value = false
    })
  } else {
    const notificationTitle = t('admin.generic.notification_warning_title');
...
onMounted(async () => {
  loadData.value = true
  await commentStore.getRecords().then(() => {
    loadData.value = false
  })
})
</script>
