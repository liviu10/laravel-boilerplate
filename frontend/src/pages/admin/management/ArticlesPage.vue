...
import TableColumns from 'src/columns/articlesColumns';
...
import { useArticleStore } from 'src/stores/admin/management/articles';
...
const getAllRecords = computed(() => articleStore.getAllRecords);
...
const getSingleRecord = computed(() => articleStore.getSingleRecord);
...
async function actionMethodDialog(action: DialogType, recordId?: number) {
  ...
  if (action === 'create') {
    ...
    if (isInvalidRecordId) {
      ...
      articleStore.findRecord(recordId).then(() => {
        loadData.value = false
        actionName.value = action
...
function handleActionMethod(action: DialogType) {
  ...
  loadData.value = true
  if (action === 'create') {
    articleStore.createRecord().then(() => {
      loadData.value = false
    })
  } else {
    const recordId = getSingleRecord.value && Object.keys(getSingleRecord.value).length > 0 ? getSingleRecord.value.id : null;
    if (recordId) {
      if (action === 'edit') {
        articleStore.updateRecord(recordId).then(() => {
          loadData.value = false
        })
      } else if (action === 'delete') {
        articleStore.deleteRecord(recordId).then(() => {
          loadData.value = false
        })
      }
    } else {
...
onMounted(async () => {
  loadData.value = true
  await articleStore.getRecords().then(() => {
    loadData.value = false
  })
})
</script>
