...
import TableColumns from 'src/columns/tagsColumns';
...
import { useTagStore } from 'src/stores/admin/management/tags';
...
const getAllRecords = computed(() => tagStore.getAllRecords);
...
const getSingleRecord = computed(() => tagStore.getSingleRecord);
...
async function actionMethodDialog(action: ActionMethodDialogType, recordId?: number) {
  ...
  if (action === 'create') {
    ...
    if (isInvalidRecordId) {
      ...
      tagStore.findRecord(recordId).then(() => {
        loadData.value = false
        actionName.value = action
...
function editRecord() {
  ...
  if (recordId) {
    loadData.value = true
    displayActionDialog.value = false
    tagStore.updateRecord(recordId).then(() => {
      loadData.value = false
    })
  } else {
    const notificationTitle = t('admin.generic.notification_warning_title');
...
function deleteRecord() {
  ...
  if (recordId) {
    loadData.value = true
    displayActionDialog.value = false
    tagStore.deleteRecord(recordId).then(() => {
      loadData.value = false
    })
  } else {
    const notificationTitle = t('admin.generic.notification_warning_title');
...
onMounted(async () => {
  loadData.value = true
  await tagStore.getRecords().then(() => {
    loadData.value = false
  })
})
</script>
