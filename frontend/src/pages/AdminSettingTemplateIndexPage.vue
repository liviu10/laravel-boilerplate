<template>
  <q-page class="admin admin--page">
    <page-title :page-title="t(`${templateStore.getTranslationString}.title`)" />

    <page-description
      :page-description="t(`${templateStore.getTranslationString}.page_description`)"
    />

    <div class="admin-section admin-section--container">
      <grid-table
        :columns="templateStore.getColumns"
        :search-resource="templateStore.getSearchResourceModel"
        :resource="templateStore.getResourceName"
        :rows="templateStore.getAllRecords.results?.data || []"
    ...
    >
      <template v-slot:dialog-details>
        <card-advanced-filter
          ...
          :data-model="templateStore.getFilterModel"
          :resource="templateStore.getResourceName"
          :translation-string="templateStore.getTranslationString"
        />

        <card-upload
          v-if="actionName === 'upload'"
          action-name="upload"
          :data-model="templateStore.getUploadModel"
          :resource="templateStore.getResourceName"
          :translation-string="templateStore.getTranslationString"
        />
        ...
        <card-download
          ...
          :data-model="templateStore.getDownloadModel"
          :resource="templateStore.getResourceName"
          :translation-string="templateStore.getTranslationString"
        />
        ...
        <card-restore
          ...
          action-name="restore"
          :record-details="templateStore.getAllDeletedRecords"
          :resource="templateStore.getResourceName"
          :translation-string="templateStore.getTranslationString"
        />
        ...
        <management-card-quick-show
          ...
          :record-details="templateStore.getSingleRecord"
          :resource="templateStore.getResourceName"
          :translation-string="templateStore.getTranslationString"
        />
        ...
        <management-card-quick-edit
          ...
          :data-model="templateStore.getDataModel"
          :resource="templateStore.getResourceName"
          :translation-string="templateStore.getTranslationString"
        >
...
import { ISingleRecord } from 'src/interfaces/TemplateInterface';
...
import { useTemplateStore } from 'src/stores/settings/templates';
...
// Get all records
templateStore.handleIndex('paginate');
...
async function handleOpenDialog(action: TDialog, recordId?: number): Promise<void> {
  ...
  switch (action) {
    ...
    case 'advanced-filters':
      ...
      if (templateStore.getFilterModel.length === 0) {
    ...
    case 'upload':
      ...
      loadPage.value = false;
      displayDialog.value = true;
      if (templateStore.getUploadModel.length === 0) {
        disableActionDialogButton.value = {
          action: action,
          disable: true,
        };
    ...
    case 'download':
      ...
      loadPage.value = false;
      displayDialog.value = true;
      if (templateStore.getDownloadModel.length === 0) {
        disableActionDialogButton.value = {
          action: action,
          disable: true,
        };
    ...
    case 'quick-show':
      actionName.value = action;
      templateStore.handleShow(recordId).then(() => {
        loadPage.value = false;
        displayDialog.value = true;
      });
      break;
    case 'quick-edit':
      actionName.value = action;
      if (templateStore.getDataModel.length === 0) {
        disableActionDialogButton.value = {
          action: action,
          disable: true,
        };
    ...
    case 'restore':
      actionName.value = action;
      templateStore.handleIndex('restore').then(() => {
        if (
          !checkObject.handleCheckIfObject(templateStore.getAllDeletedRecords) &&
          !checkObject.handleCheckIfArray(templateStore.getAllDeletedRecords.results)
        ) {
          disableActionDialogButton.value = {
            action: action,
            disable: true,
...
async function handleActionDialog(action: TDialog): Promise<void> {
  ...
  switch (action) {
    case 'create':
      templateStore.handleCreate().then(() => {
        displayDialog.value = false;
        loadPage.value = false;
      });
      break;
    case 'advanced-filters':
      templateStore.handleAdvancedFilter('paginate').then(() => {
        displayDialog.value = false;
        loadPage.value = false;
      });
      break;
    case 'upload':
      templateStore.handleUpload().then(() => {
        displayDialog.value = false;
        loadPage.value = false;
      });
      break;
    case 'download':
      templateStore.handleDownload().then(() => {
        displayDialog.value = false;
        loadPage.value = false;
      });
      break;
    case 'restore':
      templateStore.handleRestore().then(() => {
        displayDialog.value = false;
        loadPage.value = false;
      });
      break;
    case 'quick-show':
      ...
      break;
    case 'quick-edit':
      templateStore.handleUpdate().then(() => {
        displayDialog.value = false;
        loadPage.value = false;
      });
      break;
    case 'delete':
      templateStore.handleDelete().then(() => {
        displayDialog.value = false;
        loadPage.value = false;
      });
      break;
    case 'stats':
      templateStore.handleStats().then(() => {
        displayDialog.value = false;
        loadPage.value = false;
      });
      break;
...
const handleSearchTheResource = () => templateStore.handleAdvancedFilter('paginate');
...
const handleNavigateToPage = (action: TDialog) => {
  const selectedRecord: ISingleRecord = templateStore.getSingleRecord;
  const routeParams =
    selectedRecord &&
