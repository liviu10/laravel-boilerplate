// This is just an example,
// so you can safely delete all default props below

export default {
  admin: {
    generic: {
      search_the_application: 'Căutați aplicația',
      search_button_label: 'Căutare',
      notification_tooltip: 'Gestionați notificările dvs.',
      contrast_tooltip: 'Schimbați modul tematic',
      language_tooltip: 'Schimbați limba',
      english_language: 'Engleză',
      french_language: 'Franceză',
      romanian_language: 'Română',
      welcome_message: 'Bine ați venit, {username}',
      welcome_tooltip: 'Clic aici pentru setări',
      profile_label: 'Profil',
      notifications_label: 'Notificări',
      theme_mode_label: 'Mod tematic',
      logout_label: 'Deconectare',
      table_no_data_label: 'Nu exista inregistrari',
      default_title: 'Titlu implicit',
      default_page_description: `
        Lorem ipsum dolor sit amet consectetur adipisicing elit.
        Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
        Qui consequatur quidem itaque et ullam, non velit unde quo
        inventore adipisci tempora, voluptas sunt dolore vitae iure,
        quos eligendi. Corporis id adipisci aliquam cum officia impedit
        repudiandae, numquam, dicta earum, cupiditate consequuntur?
      `,
      add_new_record: 'Adăugă o înregistrare nouă',
      add_new_record_tooltip: 'Clic aici pentru a adăuga o înregistrare nouă',
      search_the_resource: 'Căutați resursa',
      resource_settings_tooltip: 'Clic aici pentru a configura tabelul',
      quick_show_record: 'Afișare rapidă înregistrare',
      quick_show_record_tooltip: 'Faceți clic aici pentru a afișa rapid înregistrarea',
      quick_edit_record: 'Editare rapidă a înregistrării',
      quick_edit_record_tooltip: 'Faceți clic aici pentru editare rapidă a înregistrării',
      delete_record: 'Șterge înregistrarea',
      delete_record_tooltip: 'Faceți clic aici pentru a șterge înregistrarea',
      advanced_filters: 'Filtre avansate',
      advanced_filters_tooltip: 'Faceți clic aici pentru a filtra înregistrările',
      default_dialog_title: 'Dialog generic',
      default_dialog_tooltip: 'Faceți clic aici pentru a vedea un dialog generic',
      close_label: 'Închide',
      cancel_label: 'Anulează',
      save_label: 'Salvează',
      edit_label: 'Editare rapidă',
      delete_label: 'Șterge',
      apply_filters_label: 'Aplică filtrele',
      ok_label: 'OK',
      options_label: 'Mai multe opțiuni',
      options_label_tooltip: 'Faceți clic aici pentru mai multe opțiuni (filtrare, încărcare, descărcare și altele)',
      configure_resource: 'Configurează resursa',
      download_label: 'Descarcă înregistrările',
      upload_label: 'Încarcă înregistrările',
      actions_label: 'Acțiuni',
      actions_label_tooltip: 'Faceți clic aici pentru acțiuni (afișare, editare, ștergere și altele)',
      content_stats: 'Statistici conținut',
      stats_label: 'Statistici',
      go_to_show: 'Mergi la pagina de detalii',
      go_to_edit: 'Mergi la pagina de editare',
      delete_confirmation_message: 'Sigur doriți să ștergeți acest înregistrare? Înregistrările șterse pot fi restaurate din Mai multe opțiuni > Restaurează înregistrările!',
      restore_label: 'Restaurează înregistrările',
      restore_confirmation_message: 'Sunteți sigur că doriți să restaurați aceste înregistrări?',
    },
    home: {
      title: 'Dashboard',
      page_description: `
        Lorem ipsum dolor sit amet, consectetur adipisicing elit.
        Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
        Qui consequatur quidem itaque et ullam, non velit unde quo
        inventore adipisci tempora, voluptas sunt dolore vitae iure,
        quos eligendi. Corporis id adipisci aliquam cum officia impedit
        repudiandae, numquam, dicta earum, cupiditate consequuntur?
      `,
      statistic_contact_messages: 'Mesaje de contact',
      statistic_newsletter_campaigns: 'Campanii de newsletter',
      statistic_newsletter_subscribers: 'Abonați la newsletter',
      statistic_reviews: 'Recenzii',
      statistic_articles: 'Vizualizări articole',
      statistic_storage: 'Stocare',
      statistic_appreciations: 'Aprecieri — Dezaprobări — Notă',
      statistic_comments: 'Comentarii',
    },
    management: {
      title: 'Administrare',
      content: {
        title: 'Conținut',
        page_description: `
          Lorem ipsum dolor sit amet consectetur adipisicing elit.
          Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
          Qui consequatur quidem itaque et ullam, non velit unde quo
          inventore adipisci tempora, voluptas sunt dolore vitae iure,
          quos eligendi. Corporis id adipisci aliquam cum officia impedit
          repudiandae, numquam, dicta earum, cupiditate consequuntur?
        `,
        data_model: {},
        no_data_model: `
          Modelul de date nu există.
          Vă rugăm să configurați resursa.
          Dacă problema persistă, puteți contacta administratorul.
        `,
      },
      tag: {
        title: 'Etichete',
        page_description: `
          Lorem ipsum dolor sit amet consectetur adipisicing elit.
          Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
          Qui consequatur quidem itaque et ullam, non velit unde quo
          inventore adipisci tempora, voluptas sunt dolore vitae iure,
          quos eligendi. Corporis id adipisci aliquam cum officia impedit
          repudiandae, numquam, dicta earum, cupiditate consequuntur?
        `,
      },
      media: {
        title: 'Media',
        page_description: `
          Lorem ipsum dolor sit amet consectetur adipisicing elit.
          Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
          Qui consequatur quidem itaque et ullam, non velit unde quo
          inventore adipisci tempora, voluptas sunt dolore vitae iure,
          quos eligendi. Corporis id adipisci aliquam cum officia impedit
          repudiandae, numquam, dicta earum, cupiditate consequuntur?
        `,
      },
      comment: {
        title: 'Comentarii',
        page_description: `
          Lorem ipsum dolor sit amet consectetur adipisicing elit.
          Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
          Qui consequatur quidem itaque et ullam, non velit unde quo
          inventore adipisci tempora, voluptas sunt dolore vitae iure,
          quos eligendi. Corporis id adipisci aliquam cum officia impedit
          repudiandae, numquam, dicta earum, cupiditate consequuntur?
        `,
      },
      appreciation: {
        title: 'Apreciere',
        page_description: `
          Lorem ipsum dolor sit amet consectetur adipisicing elit.
          Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
          Qui consequatur quidem itaque et ullam, non velit unde quo
          inventore adipisci tempora, voluptas sunt dolore vitae iure,
          quos eligendi. Corporis id adipisci aliquam cum officia impedit
          repudiandae, numquam, dicta earum, cupiditate consequuntur?
        `,
      }
    },
    communication: {
      title: 'Comunicare',
      contact: {
        title: 'Contact',
        page_description: `
          Lorem ipsum dolor sit amet consectetur adipisicing elit.
          Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
          Qui consequatur quidem itaque et ullam, non velit unde quo
          inventore adipisci tempora, voluptas sunt dolore vitae iure,
          quos eligendi. Corporis id adipisci aliquam cum officia impedit
          repudiandae, numquam, dicta earum, cupiditate consequuntur?
        `,
      },
      newsletter: {
        title: 'Buletin informativ',
        page_description: `
          Lorem ipsum dolor sit amet consectetur adipisicing elit.
          Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
          Qui consequatur quidem itaque et ullam, non velit unde quo
          inventore adipisci tempora, voluptas sunt dolore vitae iure,
          quos eligendi. Corporis id adipisci aliquam cum officia impedit
          repudiandae, numquam, dicta earum, cupiditate consequuntur?
        `,
      },
      review: {
        title: 'Recenzii',
        page_description: `
          Lorem ipsum dolor sit amet consectetur adipisicing elit.
          Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
          Qui consequatur quidem itaque et ullam, non velit unde quo
          inventore adipisci tempora, voluptas sunt dolore vitae iure,
          quos eligendi. Corporis id adipisci aliquam cum officia impedit
          repudiandae, numquam, dicta earum, cupiditate consequuntur?
        `,
      }
    },
    documentation: {
      title: 'Documentație',
      page_description: `
        Lorem ipsum dolor sit amet consectetur adipisicing elit.
        Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
        Qui consequatur quidem itaque et ullam, non velit unde quo
        inventore adipisci tempora, voluptas sunt dolore vitae iure,
        quos eligendi. Corporis id adipisci aliquam cum officia impedit
        repudiandae, numquam, dicta earum, cupiditate consequuntur?
      `,
    },
    settings: {
      title: 'Setări',
      user_profile: {
        title: 'Profil utilizator',
        page_description: `
          Lorem ipsum dolor sit amet consectetur adipisicing elit.
          Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
          Qui consequatur quidem itaque et ullam, non velit unde quo
          inventore adipisci tempora, voluptas sunt dolore vitae iure,
          quos eligendi. Corporis id adipisci aliquam cum officia impedit
          repudiandae, numquam, dicta earum, cupiditate consequuntur?
        `,
      },
      user: {
        title: 'Utilizatori',
        page_description: `
          Lorem ipsum dolor sit amet consectetur adipisicing elit.
          Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
          Qui consequatur quidem itaque et ullam, non velit unde quo
          inventore adipisci tempora, voluptas sunt dolore vitae iure,
          quos eligendi. Corporis id adipisci aliquam cum officia impedit
          repudiandae, numquam, dicta earum, cupiditate consequuntur?
        `,
      },
      roles_and_permission: {
        title: 'Roluri și permisiuni',
        page_description: `
          Lorem ipsum dolor sit amet consectetur adipisicing elit.
          Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
          Qui consequatur quidem itaque et ullam, non velit unde quo
          inventore adipisci tempora, voluptas sunt dolore vitae iure,
          quos eligendi. Corporis id adipisci aliquam cum officia impedit
          repudiandae, numquam, dicta earum, cupiditate consequuntur?
        `,
      },
      general: {
        title: 'Setări generale',
        page_description: `
          Lorem ipsum dolor sit amet consectetur adipisicing elit.
          Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
          Qui consequatur quidem itaque et ullam, non velit unde quo
          inventore adipisci tempora, voluptas sunt dolore vitae iure,
          quos eligendi. Corporis id adipisci aliquam cum officia impedit
          repudiandae, numquam, dicta earum, cupiditate consequuntur?
        `,
      },
      accepted_domain: {
        title: 'Domenii acceptate',
        page_description: `
          Lorem ipsum dolor sit amet consectetur adipisicing elit.
          Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
          Qui consequatur quidem itaque et ullam, non velit unde quo
          inventore adipisci tempora, voluptas sunt dolore vitae iure,
          quos eligendi. Corporis id adipisci aliquam cum officia impedit
          repudiandae, numquam, dicta earum, cupiditate consequuntur?
        `,
      },
      notification: {
        title: 'Notificări',
        page_description: `
          Lorem ipsum dolor sit amet consectetur adipisicing elit.
          Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
          Qui consequatur quidem itaque et ullam, non velit unde quo
          inventore adipisci tempora, voluptas sunt dolore vitae iure,
          quos eligendi. Corporis id adipisci aliquam cum officia impedit
          repudiandae, numquam, dicta earum, cupiditate consequuntur?
        `,
      },
      template: {
        title: 'Șabloane de email și SMS',
        page_description: `
          Lorem ipsum dolor sit amet consectetur adipisicing elit.
          Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
          Qui consequatur quidem itaque et ullam, non velit unde quo
          inventore adipisci tempora, voluptas sunt dolore vitae iure,
          quos eligendi. Corporis id adipisci aliquam cum officia impedit
          repudiandae, numquam, dicta earum, cupiditate consequuntur?
        `,
      },
      configuration_resource: {
        title: 'Configurare resurse',
        page_description: `
          Lorem ipsum dolor sit amet consectetur adipisicing elit.
          Iste rem est voluptatem obcaecati maiores, sit quibusdam ad.
          Qui consequatur quidem itaque et ullam, non velit unde quo
          inventore adipisci tempora, voluptas sunt dolore vitae iure,
          quos eligendi. Corporis id adipisci aliquam cum officia impedit
          repudiandae, numquam, dicta earum, cupiditate consequuntur?
        `,
      }
    }
  }
};

