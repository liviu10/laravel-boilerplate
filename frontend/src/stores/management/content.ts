...
export const useContentStore = defineStore('contentStore', () => {
  ...
  async function handleIndex(type?: TResourceType) {
    try {
      handleApi.getEndpoint(getResourceName.value, useContentStore.$id).then(
        async (apiEndpoint) => {
          if (apiEndpoint) {
            handleApi.getConfiguration(getResourceName.value, useContentStore.$id).then(
  ...
  async function handleAdvancedFilter(type?: TResourceType) {
    ...
    try {
      handleApi.getEndpoint(resourceName.value, useContentStore.$id).then(
        async (apiEndpoint) => {
          if (apiEndpoint) {
            resourceEndpoint.value = apiEndpoint[0].path
            const response = await api.get(resourceEndpoint.value, {
