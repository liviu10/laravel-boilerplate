...
export const useAppreciationStore = defineStore('appreciationStore', () => {
  ...
  async function handleIndex(type?: TResourceType) {
    try {
      handleApi.getEndpoint(getResourceName.value, useAppreciationStore.$id).then(
        async (apiEndpoint) => {
          if (apiEndpoint) {
            handleApi.getConfiguration(getResourceName.value, useAppreciationStore.$id).then(
  ...
  async function handleAdvancedFilter(type?: TResourceType) {
    ...
    try {
      handleApi.getEndpoint(resourceName.value, useAppreciationStore.$id).then(
        async (apiEndpoint) => {
          if (apiEndpoint) {
            resourceEndpoint.value = apiEndpoint[0].path
            const response = await api.get(resourceEndpoint.value, {
