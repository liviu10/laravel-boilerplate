...
import { IAllRecords, IAllRecordsUnpaginated, ISingleRecord } from 'src/interfaces/CommentInterface'
...
export const useCommentStore = defineStore('commentStore', () => {
  ...
  async function handleIndex(type?: TResourceType) {
    try {
      handleApi.getEndpoint(getResourceName.value, useCommentStore.$id).then(
        async (apiEndpoint) => {
          if (apiEndpoint) {
            handleApi.getConfiguration(getResourceName.value, useCommentStore.$id).then(
  ...
  async function handleAdvancedFilter(type?: TResourceType) {
    ...
    try {
      handleApi.getEndpoint(resourceName.value, useCommentStore.$id).then(
        async (apiEndpoint) => {
          if (apiEndpoint) {
            resourceEndpoint.value = apiEndpoint[0].path
            const response = await api.get(resourceEndpoint.value, {
