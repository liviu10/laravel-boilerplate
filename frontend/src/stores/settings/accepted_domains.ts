...
import { IAllRecords, IAllRecordsUnpaginated, ISingleRecord } from 'src/interfaces/AcceptedDomainInterface'
...
export const useAcceptedDomainStore = defineStore('acceptedDomainStore', () => {
  ...
  async function handleIndex(type?: TResourceType) {
    try {
      handleApiResource.apiEndpoint(resourceName, useAcceptedDomainStore.$id).then(
        async (apiEndpoint) => {
          if (apiEndpoint) {
