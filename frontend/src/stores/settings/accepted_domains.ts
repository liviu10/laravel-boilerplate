...
import {
  ...
  ISingleRecord,
} from 'src/interfaces/AcceptedDomainInterface';
...
export const useAcceptedDomainStore = defineStore('acceptedDomainStore', () => {
  // State
  const resourceName = 'Accepted domains';
  ...
  const columns: Ref<QTableProps['columns']> = ref(
    defaultColumns as QTableProps['columns']
  );
  const dataModel: Ref<IConfigurationInput[]> = ref(
    defaultDataModel as IConfigurationInput[]
  );
  const filterModel: Ref<IConfigurationInput[]> = ref(
    defaultFilterModel as IConfigurationInput[]
  );
  const uploadModel: Ref<IConfigurationInput[]> = ref(
    defaultUploadModel as IConfigurationInput[]
  );
  const downloadModel: Ref<IConfigurationInput[]> = ref(
    defaultDownloadModel as IConfigurationInput[]
  );
