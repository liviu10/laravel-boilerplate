...
import { IAllRecords, IAllRecordsUnpaginated, ISingleRecord } from 'src/interfaces/UserInterface'
...
export const useUserStore = defineStore('userStore', () => {
  ...
  async function handleIndex(type?: TResourceType) {
    try {
      handleApiResource.apiEndpoint(resourceName, useUserStore.$id).then(
        async (apiEndpoint) => {
          if (apiEndpoint) {
